...
/**
 * This class represents a puzzle simulator with tiles, including initial and
 * final boards,
 * movable tiles, holes, and glue. It allows operations such as adding,
...
public class Puzzle {
    ...
    // Lists of lists of tiles representing the current board state and reference
    // state
    ...
    // Queue that will be useful to store tiles with fragileGlue (reason because of
    // tilt), after tilt, all tiles must remove glue
     ...
     * @throws PuzzleExceptions.ExceedPuzzleSpaceException if the specified height
     *                                                     or width exceeds 500.
     * @throws PuzzleExceptions.ConstructorsExceptions     if the specified height
     *                                                     or width is less than 1.
     */
    public Puzzle(int h, int w)
            throws puzzle.PuzzleExceptions.ExceedPuzzleSpaceException, puzzle.PuzzleExceptions.ConstructorsExceptions {
        try {
            if (h < 1 || w < 1)
                throw new PuzzleExceptions.ConstructorsExceptions(
                        PuzzleExceptions.ConstructorsExceptions.NO_BOARDS_HW_NEGATIVE);
            if (h > 500 || w > 500)
                throw new PuzzleExceptions.ExceedPuzzleSpaceException(
                        PuzzleExceptions.ExceedPuzzleSpaceException.EXCEED_PUZZLE_SPACE);
            this.h = h;
            this.w = w;
            ...
            // Create empty char list to be used as board argument to create both boards
            // empty
    ...
    /**
     * Constructor to initialize the boards with initial and final character
     * matrices.
     * 
    ...
    public Puzzle(char[][] starting, char[][] ending) throws puzzle.PuzzleExceptions.ConstructorsExceptions {
        try {
            if (starting == null || ending == null)
                throw new PuzzleExceptions.ConstructorsExceptions(
                        PuzzleExceptions.ConstructorsExceptions.NO_STARTING_ENDING_NULL);
    ...
    /**
     * Constructor to initialize an empty initial board and a final board with
     * tiles.
     * 
     * @param ending Final state of the board.
     */
    public Puzzle(char[][] ending) throws puzzle.PuzzleExceptions.ConstructorsExceptions {
        try {
            if (ending == null)
                throw new PuzzleExceptions.ConstructorsExceptions(
                        PuzzleExceptions.ConstructorsExceptions.NO_ENDING_NULL);
     ...
     * @param startingBoard boolean value to determinate if it is the starting or
     *                      ending board
     */
    ...
    /**
     * This method takes a string formatted as "[type label]", where the first
     * character(s)
     * represent the type of tile and the second part is the label of the tile.
     *
     * @param tileData  A string containing the type and label of the tile,
     *                  formatted as "[type label]".
     ...
     * @return An instance of a subclass of BaseTile based on the type specified in
     *         tileData.
     ...
     * @throws PuzzleExceptions.addDeleteTileExceptions    if the tile data is
     *                                                     invalid or if the
     *                                                     position is invalid.
     * @throws PuzzleExceptions.ExceedPuzzleSpaceException if the specified position
     *                                                     exceeds the puzzle
     *                                                     dimensions.
     */
    public void addTile(int row, int column, char label)
            throws PuzzleExceptions.addDeleteTileExceptions, PuzzleExceptions.ExceedPuzzleSpaceException {
        try {
            ...
            if (!isValidLabel) {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_VALID_LABEL);
            }
            ...
            if (row >= h || column >= w) {
                ...
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_NEGATIVE_POSITION_TILE);
            }
            ...
            if (previousTile.getLabel() == 'h') {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_ADD_HOLE_TILE);

            } else if (!isTileEmpty(previousTile)) {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.TILE_OCCUPIED);
            }
    ...
    /**
     * Adds a tile to the board at the specified position using tile data in string
     * format.
     * This method is overloaded to allow the creation of different types of tiles.
     *
     * @param row      Row index of the tile.
     * @param column   Column index of the tile.
     * @param tileData A string containing the type and label of the tile, formatted
     *                 as "[type label]".
     * @throws PuzzleExceptions.addDeleteTileExceptions    if the tile data is
     *                                                     invalid or if the
     *                                                     position is invalid.
    ...
    public void addTile(int row, int column, String tileData)
        ...
        try {
            ...
            if (!tileType.equals("fr") && !tileType.equals("fl") && !tileType.equals("ro") && !tileType.equals("fi")
                    && !tileType.equals("wi")) {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_EXISTENT_TYPE_TILE);
            }
            ...
            if (tileType.equals("fr"))
                newTile.setLabel('l');
            if (tileType.equals("fl"))
                newTile.setLabel('f');
            if (tileType.equals("ro"))
                newTile.setLabel('o');
            if (tileType.equals("fi"))
                newTile.setLabel('x');
     ...
     * @param row    Row index of the tile.
     * @param column Column index of the tile.
     * @throws PuzzleExceptions.addDeleteTileExceptions    if the tile cannot be
     *                                                     deleted due to specific
     *                                                     conditions.
     * @throws PuzzleExceptions.ExceedPuzzleSpaceException if the specified position
     *                                                     exceeds the puzzle
     *                                                     dimensions.
    ...
    public void deleteTile(int row, int column)
        ...
        try {
            ...
            if (previousTile.getLabel() == 'h') {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_DELETE_HOLE_TILE);
            } else if (previousTile.hasGlue() || previousTile.isStuck()) {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_DELETE_TILE_GLUE);
            } else if (previousTile instanceof FixedTile) {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_DELETE_FIXED_TILE);
            } else if (previousTile instanceof WildTile) {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_DELETE_WILD_TILE);
            } else if (isTileEmpty(previousTile)) {
                throw new PuzzleExceptions.addDeleteTileExceptions(
                        PuzzleExceptions.addDeleteTileExceptions.NOT_DELETE_NON_EXISTENT_TILE);
            }
     ...
     * 
     * @param from the coordinates of the source position as an integer array [row,
     *             col].
     * @param to   the coordinates of the destination position as an integer array
     *             [row, col].
     * @throws PuzzleExceptions.relocateTileExceptions if the relocation cannot be
     *                                                 performed due to specific
     *                                                 conditions.
     */
    public void relocateTile(int[] from, int[] to) throws PuzzleExceptions.relocateTileExceptions {
        try {
            ...
            if (!areValidCoordinates(from) || !areValidCoordinates(to)) {
                throw new PuzzleExceptions.relocateTileExceptions(
                        PuzzleExceptions.relocateTileExceptions.INVALID_COORDINATES);
            }

            BaseTile fromTile = tiles.get(from[0]).get(from[1]);
            BaseTile toTile = tiles.get(to[0]).get(to[1]);

            // Validate existence of the source tile and availability of the destination
            // tile
            if (fromTile.getLabel() == 'h') {
                throw new PuzzleExceptions.relocateTileExceptions(
                        PuzzleExceptions.relocateTileExceptions.NOT_MOVE_HOLE_TILE);
            } else if (toTile.getLabel() == 'h') {
                throw new PuzzleExceptions.relocateTileExceptions(
                        PuzzleExceptions.relocateTileExceptions.NOT_RELOCATE_TILE_HOLE);
            } else if (isTileEmpty(fromTile)) {
                throw new PuzzleExceptions.relocateTileExceptions(
                        PuzzleExceptions.relocateTileExceptions.NOT_MOVE_NON_EXISTENT_TILE);
            } else if (!isTileEmpty(toTile)) {
                throw new PuzzleExceptions.relocateTileExceptions(
                        PuzzleExceptions.relocateTileExceptions.NOT_RELOCATE_TILE_OCCUPIED);
            } else if (fromTile.hasGlue() || fromTile.isStuck()) {
                throw new PuzzleExceptions.relocateTileExceptions(
                        PuzzleExceptions.relocateTileExceptions.NOT_MOVE_TILE_GLUE);
            } else if (fromTile instanceof FixedTile) {
                throw new PuzzleExceptions.relocateTileExceptions(
                        PuzzleExceptions.relocateTileExceptions.NOT_RELOCATE_FIXED_TILE);
            }
    ...
    /**
     * Performs the visual and logical movement of a tile from one position to
     * another.
     * 
     ...
     * @throws PuzzleExceptions.addDeleteGlueExceptions if the glue cannot be
     *                                                  applied due to specific
     *                                                  conditions.
     */
    public void addGlue(int row, int column) throws PuzzleExceptions.addDeleteGlueExceptions {
        try {
            ...
            if (tile == null) {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.INVALID_POSITION);

            } else if (tile.getLabel() == 'h') {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_ADD_GLUE_HOLE);

            } else if (isTileEmpty(tile)) {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_ADD_GLUE_EMPTY_TILE);

            } else if (tile.hasGlue()) {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_GLUE_EXISTING_TILE);

            } else if (tile instanceof FreelanceTile) {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_GLUE_FREELANCE_TILE);
            }
    ...
    public void addGlue(int row, int column, String glueType) throws PuzzleExceptions.addDeleteGlueExceptions {
        try {
            ...
            if (!glueType.equalsIgnoreCase("fragile") && !glueType.equalsIgnoreCase("super")) {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_VALID_TYPES_GLUE);
            }
    ...
    public void deleteGlue(int row, int column) throws PuzzleExceptions.addDeleteGlueExceptions {
        try {
            ...
            if (tile == null) {
                ...
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_DELETE_GLUE_HOLE);

            } else if (isTileEmpty(tile)) {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_DELETE_GLUE_EMPTY_TILE);

            } else if (!tile.hasGlue()) {
                throw new PuzzleExceptions.addDeleteGlueExceptions(
                        PuzzleExceptions.addDeleteGlueExceptions.NOT_GLUE_TO_REMOVE);
            }
     ...
     * @return {@code true} if adjacent to a tile with glue; {@code false}
     *         otherwise.
     */
     ...
     * 
     * @param direction the direction to tilt ('d' for down, 'u' for up, 'r' for
     *                  right, 'l' for left).
     * @throws PuzzleExceptions.addDeleteGlueExceptions Validate fragile glue when
     *                                                  tilt
     */
     ...
     * @param group          The group of tiles that are considered to be moving
     *                       together.
     * @param isGluedOrStuck Indicates if the tile is glued or stuck.
     * @return The maximum number of steps the tile can move up, or -1 if it would
     *         fall into a hole.
     */
     ...
     * @return The maximum number of steps the group can move up, or -1 if any tile
     ...
     *                       together.
     * @param isGluedOrStuck Indicates if the tile is glued or stuck.
     * @return The maximum number of steps the tile can move down, or -1 if it would
     *         fall into a hole.
     */
     ...
     *                       together.
     * @param isGluedOrStuck Indicates if the tiles are glued or stuck.
     * @return The maximum number of steps the group can move down, or -1 if any
     *         tile would fall into a hole.
     */
     ...
     *                       together.
     * @param isGluedOrStuck Indicates if the tile is glued or stuck.
     * @return The maximum number of steps the tile can move left, or -1 if it would
     *         fall into a hole.
     */
     ...
     *                       together.
     * @param isGluedOrStuck Indicates if the tiles are glued or stuck.
     * @return The maximum number of steps the group can move left, or -1 if any
     *         tile would fall into a hole.
     */
     ...
     *                       together.
     * @param isGluedOrStuck Indicates if the tile is glued or stuck.
     * @return The maximum number of steps the tile can move right, or -1 if it
     *         would fall into a hole.
     */
     ...
     *                       together.
     * @param isGluedOrStuck Indicates if the tiles are glued or stuck.
     * @return The maximum number of steps the group can move right, or -1 if any
     *         tile would fall into a hole.
     */
    ...
    /**
     * Collects all tiles that are stuck together as a group, starting from the
     * given tile.
    ...
    public boolean isGoal() {
        // Move in in the actual board (tiles) and compare it with the reference
        // board(ending)
        for (int row = 0; row < h; row++) {
            for (int col = 0; col < w; col++) {
                ...
                if (currentLabel == 'h' || targetLabel == 'h' || currentLabel == 'l' || targetLabel == 'l'
                        || currentLabel == 'f' || targetLabel == 'f' || currentLabel == 'x' || targetLabel == 'x'
                        || currentLabel == 'o' || targetLabel == 'o' || currentLabel == 'p' || targetLabel == 'p'
                        || currentLabel == 'w' || targetLabel == 'w') {
                    continue;
                }
        ...
        // If all tiles assimilates with the reference tiles, so we reached the final
        // state
        this.ok = true;
        return true;
    }
     ...
     * 
     * @throws PuzzleExceptions.makeVisibleInvisibleExceptions exceptions for
     *                                                         starting and ending
     *                                                         board
     */
    ...
    public void makeVisible() throws PuzzleExceptions.makeVisibleInvisibleExceptions {
        try {
            if (startingBoard == null)
                throw new PuzzleExceptions.makeVisibleInvisibleExceptions(
                        PuzzleExceptions.makeVisibleInvisibleExceptions.NO_STARTING_BOARD_NULL);
            if (endingBoard == null)
                throw new PuzzleExceptions.makeVisibleInvisibleExceptions(
                        PuzzleExceptions.makeVisibleInvisibleExceptions.NO_ENDING_BOARD_NULL);
    ...
    /**
     * Returns a copy of the current puzzle board (starting), representing the
     * current state.
     *
    ...
    public void exchange() {
        ...
        this.ok = true;
        System.out.println(
                "Boards have been exchanged. Now, you're editing the board that was the reference board before.");
    }
     ...
     * @param row    Row index
     * @param column Column index
     * @throws PuzzleExceptions.makeHoleExceptions         if a hole cannot be
     *                                                     created due to specific
     *                                                     conditions.
     * @throws PuzzleExceptions.ExceedPuzzleSpaceException if the specified position
     *                                                     exceeds the puzzle
     *                                                     boundaries.
    ...
    public void makeHole(int row, int column)
            throws PuzzleExceptions.makeHoleExceptions, PuzzleExceptions.ExceedPuzzleSpaceException {
        try {
            ...
            if (row >= h || column >= w) {
                ...
                throw new PuzzleExceptions.makeHoleExceptions(
                        PuzzleExceptions.makeHoleExceptions.NOT_NEGATIVE_POSITION_HOLE);
            }
            ...
            if (targetTile.getLabel() == 'h') {
                ...
                throw new PuzzleExceptions.makeHoleExceptions(
                        PuzzleExceptions.makeHoleExceptions.NOT_HOLE_TILE_OCCUPIED);
            }
    ...
    /**
     * Smart tilt method that performs an intelligent tilt to bring the puzzle
     * closer to the solution.
     * 
     * @throws PuzzleExceptions.addDeleteGlueExceptions tilt in an intelligent way
     *                                                  needs to be handled
     */
    public void tilt() throws PuzzleExceptions.addDeleteGlueExceptions {
        ...
        // Use BFS to find the sequence of moves from the current configuration to the
        // solution
    ...
    /**
     * BFS implementation to find the sequence of tilts from the current
     * configuration to the solution.
     */
     ...
     * 
     * @return A matrix of fixed tiles, where 0 indicates a fixed tile and 1
     *         indicates a movable tile.
     */
     ...
     * @return true if an empty tile or a hole is found in the column, false
     *         otherwise.
     */
